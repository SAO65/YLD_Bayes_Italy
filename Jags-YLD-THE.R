...
ID <- "ITA"
xpoint <- data$yld_2010[data$country == ID]
ypoint <- data$the_2010[data$country == ID]
...
plotMCMC = function( codaSamples , data , xName=x_Name, yName=y_Name,
                     ...
                     saveName="YLD-THE-Bayes-Italy-" , saveType=imageCompressionFormat ) {
  ...
  plot( x , y , cex=1.5 , lwd=2 , col="black" , xlim=xLim , ylim=yLim ,
        xlab="Number of Years Lived with Disease" , 
        ylab="Per-capita Total Health Expenditure" , 
        cex.lab=1.5 ,
  ...
  points( x , y , cex=1.5 )
  # add ITALY in red + label:
  points(xpoint, ypoint, col="red", cex=1.5, pch=22, lty = "solid", lwd = 2)
  text(xpoint, ypoint, labels="ITALY", cex= 0.7, pos=4)
  if ( !is.null(saveName) ) {
